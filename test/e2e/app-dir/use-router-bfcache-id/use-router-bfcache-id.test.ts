...
describe('use-router-bfcache-id', () => {
  ...
  it('preserves form state across hash navigation', async () => {
    const { browser, act } = await setup('/x/1')
    await browser.elementByCss('[data-testid="leaf-input"]').type('hello')

    await act(async () => {
      await browser
        .elementByCss('input[data-link-accordion="/x/1#section"]')
        .click()
      await browser.elementByCss('a[href="/x/1#section"]').click()
    })
